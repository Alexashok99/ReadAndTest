...
class Settings:
    ...
    #-------- AI Related--------------------
    AVAILABLE_MODELS = ["gemini-2.5-flash", "gemini-1.5-flash", "gemini-2.5-pro", "gemini-2.5-flash-lite"]
...
class HomePage(BaseFrame):
    ...
    def create_button_area(self):
        ...
        ttk.Button(button_frame, 
                   text="Quiz Time", 
                   bootstyle=Settings.SUCCESS_OUTLINE, 
                   width=Settings.SM_BTN_WIDTH, command=lambda: (self.app.switch_frame(QuizPage) if self.app else None)
                   ).pack(pady=Settings.PAD_MEDIUM, fill=X)
...
class QuizAttemptLeftFrame(BaseFrame):
    ...
    def _display_summary(self, results: Dict[str, Any]) -> None:
        ...
        ttk.Label(
            ...
            bootstyle="success"
        ).pack(anchor="center", padx=Settings.PAD_LARGE, pady=(10, 15))
    ...
    def show_api_key_dialog(self, from_menu: bool = False):
        ...
        def save_and_close():
            ...
            if key:
                ...
                dialog.destroy()
                if not from_menu and not any(isinstance(w, HomePage) for w in self.winfo_children()):
                    self.main_frame()
